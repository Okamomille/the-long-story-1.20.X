...
public class BinaryTallPlantBlock
        extends BinaryPlantBlock {
    public static final EnumProperty<DoubleBlockHalf> HALF = Properties.DOUBLE_BLOCK_HALF;

    public BinaryTallPlantBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        ...
        world.setBlockState(blockPos, BinaryTallPlantBlock.withWaterloggedState(world, blockPos, (BlockState)this.getDefaultState().with(HALF, DoubleBlockHalf.UPPER)), Block.NOTIFY_ALL);
    }
    ...
    public static void placeAt(WorldAccess world, BlockState state, BlockPos pos, int flags) {
        BlockPos blockPos = pos.up();
        world.setBlockState(pos, BinaryTallPlantBlock.withWaterloggedState(world, pos, (BlockState)state.with(HALF, DoubleBlockHalf.LOWER)), flags);
        world.setBlockState(blockPos,BinaryTallPlantBlock.withWaterloggedState(world, blockPos, (BlockState)state.with(HALF, DoubleBlockHalf.UPPER)), flags);
    }
    ...
    public void onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (!world.isClient) {
            if (player.isCreative()) {
                BinaryTallPlantBlock.onBreakInCreative(world, pos, state, player);
            } else {
                BinaryTallPlantBlock.dropStacks(state, world, pos, null, player, player.getMainHandStack());
            }
        }
